...
export default function attachBackgroundHooks(bridge /* , allActiveConnections */ ) {
    ...
    bridge.on('test', d => {
            ...
            let current_epoch = current_date.valueOf()
            let last_30_day = new Date().setDate(current_date.getDate() - 30)
            let last_30_day_epoch = last_30_day.valueOf()

            chrome.history.search({ text: 'quasar', maxResults: 100, startTime: current_epoch, endTime: last_30_day_epoch }, (data) => {
                data.forEach(function(page) {
                    console.log(page);
                });
                // bridge.send(d.eventResponseKey, { payload: data })
